<template>
    <div class="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
      ...
      <div v-if="isConnected" class="space-y-4">
        ...
        <div class="p-4 bg-green-50 rounded-xl border border-green-200">
          <div class="flex items-center justify-between mb-3">
            <div>
              <p class="font-semibold text-green-800">Seu Bot Pessoal</p>
          ...
          <!-- Usage Instructions -->
          <div class="space-y-2">
            <p class="text-sm text-green-800 font-medium">Como usar:</p>
        ...
        <button
          ...
          class="w-full py-2 text-sm text-red-600 hover:text-red-700 disabled:opacity-50 transition-colors"
        >
      ...
      <div v-else class="space-y-4">
        ...
        <button
          @click="handleConnect"
          :disabled="connecting"
          class="w-full bg-green-600 text-white py-3 px-4 rounded-lg font-medium hover:bg-green-700 disabled:opacity-50 transition-colors flex items-center justify-center space-x-2"
        >
          <svg v-if="connecting" class="animate-spin -ml-1 mr-2 h-4 w-4 text-white" fill="none" viewBox="0 0 24 24">
  ...
  // Computed
  const isConnected = computed(() => props.whatsappSession?.is_active)
  
  const connectionStatus = computed(() => {
    if (!props.whatsappSession) return 'Desconectado'
    return props.whatsappSession.is_active ? 'Conectado' : 'Desconectado'
  })
  
  const whatsappLink = computed(() => {
    if (!props.whatsappSession?.bot_number) return '#'
    const cleanNumber = props.whatsappSession.bot_number.replace(/\D/g, '')
  ...
  const handleConnect = async () => {
    ...
    try {
      router.visit('/whatsapp/connect', {
        onSuccess: () => {
          // Será redirecionado para página de conexão
        },
        onError: (errors) => {
          console.error('Erro ao conectar:', errors)
